...
class SobelEdgeLoss(nn.Module):
    ...
    def forward(self, pred, target):
        c = pred.size(1)
        kx = self.kernel_x.repeat(c, 1, 1, 1).to(pred.device, pred.dtype)
        ky = self.kernel_y.repeat(c, 1, 1, 1).to(pred.device, pred.dtype)
...
class FFTLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        return F.l1_loss(pred_mag, target_mag).to(pred.dtype)
...
class SSIMLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        return (0.6 * loss_scale1 + 0.4 * loss_scale2).to(pred.dtype)
